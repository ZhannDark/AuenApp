...
class AuthService{
    ...
    /// - Parameters:
    ///   - userRequest: user information(email, username, password)
    ///   - completion: completion with two values...
    ///   - Bool: wasRegistered - Determines if the user was registerd and saved in the firebase database correctly
    ///   - Error?: Optional error if database provides once
    ///
    public func registerUser(with userRequest: RegisterUserRequest, completion: @escaping (Bool, Error?)->Void) {
        ...
        Auth.auth().createUser(withEmail: email, password: password) { result, error in
            ...
            db.collection("users").document(resultUser.uid).setData(["username": username, "email": email]) {
                error in
                if let error = error{
                    completion(false, error)
                    return
