...
class RegisterController: UIViewController {
    ...
    private let usernameField = UserTextField(fieldtype: .username)
    private let emailField = UserTextField(fieldtype: .email)
    private let passwordField = UserTextField(fieldtype: .password)
    
    private let signUpButton = UserButton(title: "Register", hasBackground: true, fontSize: .big)
    private let signInButton = UserButton(title: "Already have an account? Sign in.", fontSize: .med)
    ...
    private func setupUI() {
        ...
        NSLayoutConstraint.activate([
            ...
            self.signInButton.topAnchor.constraint(equalTo: termsTextView.bottomAnchor),
    ...
    @objc func didTapSignUp() {
        ...
        if !Validator.isValidUsername(for: registerUserRequest.username) {
            AlertMessage.showInvalidUsernameAlert(on: self)
            return
        }
        ...
        AuthService.shared.registerUser(with: registerUserRequest) { [weak self] wasRegistered, error in
            ...
            if let error = error {
                AlertMessage.showInvalidRegistrationAlert(on: self, with: error)
                return
            }
            ...
            if wasRegistered {
                ...
                AlertMessage.showInvalidRegistrationAlert(on: self)
            }
        }
    }
...
extension RegisterController: UITextViewDelegate {
    ...
    func textView(_ textView: UITextView, shouldInteractWith URL: URL, in characterRange: NSRange, interaction: UITextItemInteraction) -> Bool {
        ...
        if URL.scheme == "terms" {
            self.showTermsView(with: "https://policies.google.com/terms?hl=en-US")
        } else if URL.scheme == "privacy" {
            self.showTermsView(with: "https://policies.google.com/privacy?hl=en-US")
        }
        
        return true
    }
    
    private func showTermsView(with urlString: String) {
        let vc = TermsViewController(with: urlString)
